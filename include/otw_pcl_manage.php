<?php
	...
	$page_title = __( 'Create New overlay', 'otw_pcl' );
	...
	if( isset( $_GET['overlay'] ) ){
		...
		if( is_array( $otw_overlays ) && isset( $otw_overlays[ $otw_overlay_id ] ) ){
			...
			$page_title = __( 'Edit overlay', 'otw_pcl' );
		}
	}
...
<div class="wrap">
	...
	<h2>
		<?php echo $page_title; ?>
		<a class="preview button" href="admin.php?page=otw-pcl"><?php _e('Back To Available overlays', 'otw_pcl')?></a>
	</h2>
	<?php include_once( 'otw_pcl_help.php' );?>
	<div class="updated <?= $info_box_class?>">
		<div class="otw_info_box_controls">
			<a href="javascript:;" class="otw_open_info_box"><?php _e( 'info', 'otw_pcl' )?></a>
			<a href="javascript:;" class="otw_close_info_box"><?php _e( 'x', 'otw_pcl' )?></a>
		</div>
		<div class="otw_info_box_content">
			<p><?php _e( 'Wondering how this works?', 'otw_pcl' );?></p>
			<ul>
				<li><?php _e( '1. Choose the Overlay Type', 'otw_pcl' );?></li>
				<li><?php _e( '2. Set it up in <a href="javascript:;" class="otw_scrollto" rel="otw_options_main">Overlay Options</a> bellow', 'otw_pcl' );?></li>
				<li><?php _e( '3. If you need to style it add a custom css class in <a href="javascript:;" class="otw_scrollto" rel="otw_options_custom">Custom Styling Options</a> bellow', 'otw_pcl' );?></li>
				<li><?php _e( '4. Go down to the <a href="javascript:;" rel="otw_grid_manager_content" class="otw_scrollto">OTW Grid Manager Light</a> to create a layout and shorcodes in your overlay. The layout is the skeleton that holds the shortcodes and makes sure everything is well arranged.', 'otw_pcl' );?></li>
			</ul>
		</div>
	</div>
				...
				<div id="post-body-content">
					<div id="otw-col-left">
						<div class="form-field form-required">
							<label for="title"><?php _e( 'Overlay Title', 'otw_pcl' );?></label>
							<input type="text" id="title" value="<?php echo $otw_overlay_values['title']?>" tabindex="1" size="30" name="title"/>
							<p><?php _e( 'The name is how it appears on your admin panel.', 'otw_pcl' );?></p>
						</div>
						<div class="form-field">
							<label for="status"><?php _e( 'Status', 'otw_pcl' );?></label>
							<select id="status" tabindex="2" style="width: 170px;" name="status">
								<option value="active"<?php if( $otw_overlay_values['status'] == 'active' ){ echo ' selected="selected" ';}?>><?php _e( 'Active', 'otw_pcl' )?></option>
								<option value="inactive"<?php if( $otw_overlay_values['status'] == 'inactive' ){ echo ' selected="selected" ';}?>><?php _e( 'Inactive', 'otw_pcl' )?></option>
							</select>
						</div>
						<div class="form-field form-required">
							<label for="type"><?php _e( 'Overlay Type', 'otw_pcl' );?></label>
							...
							<p><?php _e( 'Choose the type of your overlay.', 'otw_pcl' );?></p>
						</div>
						<?php foreach( $otw_pcl_overlay_object->overlay_types as $overlay_type => $overlay_data ){?>
										...
										<?php if( in_array( $option_name, array( 'main', 'custom' ) ) ){?>
											<div class="meta-box-sortables otw_options_<?= $option_name?>">
												<div data-type="<?= $option_name?>" class="postbox<?php echo ( $page_settings['metabox_'.$option_name ] == 'closed'  )?' closed':''?>">
													<div title="<?php _e('Click to toggle', 'otw_pcl')?>" class="handlediv sitem_toggle"><br /></div>
						...
						<div>
							<p class="submit">
								<input type="submit" value="<?php _e( 'Save Overlay', 'otw_pcl') ?>" name="submit" class="button button-primary button-large"/>
							</p>
						</div>
					</div>
					<div id="otw-col-right" class="otw_pcl_<?php echo $otw_overlay_id?>">
						
					</div>
					<div id="<?php echo $otw_pcl_grid_manager_object->meta_name;?>">
						<div class="meta-box-sortables">
							<div class="postbox">
								<h3 class="hndle sitem_header"><span><?php _e( 'OTW Grid Manager Light', 'otw_pcl' )?></span></h3>
								<div class="inside">
